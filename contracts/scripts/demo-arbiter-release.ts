...
function pickSig(iface: ethers.Interface, candidates: Array<{ name: string; args: string[] }>) {
  ...
  throw new Error(`No matching arbiter-release function found. ABI functions: ${names}`);
}
...
async function main() {
  ...
  if (!escrowId.startsWith("0x") || escrowId.length !== 66) {
    ...
    console.log("ESCROW_ID=0x... npx hardhat run scripts/demo-arbiter-release.ts --network baseSepolia");
    process.exit(1);
  }
  ...
  // Pattern A: direct arbiterRelease(bytes32)
  // Pattern B: resolveDispute(bytes32,bool) with true meaning release payee
  let data: string | null = null;
  let chosen: string | null = null;

  try {
    chosen = pickSig(escrow.interface, [{ name: "arbiterRelease", args: ["bytes32"] }]);
  ...
  if (!data) {
    try {
      ...
      data = escrow.interface.encodeFunctionData(chosen, [escrowId, true]);
    } catch {}
  }

  if (!data || !chosen) {
    chosen = pickSig(escrow.interface, [
      { name: "releaseByArbiter", args: ["bytes32"] },
